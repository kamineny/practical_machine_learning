...
#Loop "Ã  la Python" style (sorry, kind of in a hurry.. IMPROVE WITH APPLY FUNCTION IF TIME)
...
for(i in 1:length(testingV2)) { #for every column in the "improved" testing dataset
	if( sum( is.na( testingV2[, i] ) ) /nrow(testingV2) >= .6 ) { #if nÂº NAs > 60% of total observations
...
for(i in 1:length(validationV2)) { #for every column in the real Testing dataset, that I called Validation set
	if( sum( is.na( validationV2[, i] ) ) /nrow(validationV2) >= .6 ) { #if nÂº NAs > 60% of total observations
